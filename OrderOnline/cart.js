/* checks whether our document is loaded or not before we actual try to access differnt parts to it */
if (document.readyState == 'loading') { /* document.readyState just returns the current status */
    document.addEventListener('DOMContentLoaded', ready) /* Attaches an event handler to the document */
} else {
    ready()
}

function ready() {
    var addToCartButtons = document.getElementsByClassName('shop-item-button') /* getElementsByClassName just returns the elements in the document with specied class */
    for (var i = 0; i < addToCartButtons.length; i++) {
        var button = addToCartButtons[i]
        button.addEventListener('click', addToCartClicked) /* addEventListener Click is used when we click on element it just add the items  */
    }

    var removeCartItemButtons = document.getElementsByClassName('btn-danger')
    for (var i = 0; i < removeCartItemButtons.length; i++) {
        var button = removeCartItemButtons[i]
        button.addEventListener('click', removeCartItem) /* addEventListener Click is used when we click on element  it removes the items  */
    }
